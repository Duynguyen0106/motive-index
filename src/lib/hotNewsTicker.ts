...
function scoreNewsItem(item: WorldNewsItem): number {
...
function isBreakingHeadline(item: WorldNewsItem): boolean {
  const text = `${item.headline} ${item.summary}`.toLowerCase();
  return BREAKING_KEYWORDS.some((kw) => text.includes(kw));
}

function toHotItem(item: WorldNewsItem, score: number): HotCrimeNewsItem {
  const region =
    item.region
      ? item.region
      : item.country
  ...
  return {
    ...
    region: item.region ? formatNewsRegion(item) : region,
...
/** Rank world crime news for the hot ticker and News tab (score ≥ 6). */
export function detectHotCrimeNews(
  worldNewsItems: WorldNewsItem[],
  limit = 6,
): HotCrimeNewsItem[] {
  return worldNewsItems
