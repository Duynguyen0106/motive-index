import Link from "next/link";
import { COUNTRY_LABELS } from "@/lib/country";
import { searchUrlFromFilters } from "@/lib/search";
import type { SearchFilters } from "@/lib/types";
import {
  CRIME_CATEGORY_LABELS,
  DOCUMENT_TYPE_LABELS,
  FACTOR_LABELS,
  FRAMEWORK_LABELS,
} from "@/lib/types";

function withoutFilter(filters: SearchFilters, key: keyof SearchFilters): string {
  const next = { ...filters, [key]: key === "catalogTier" ? "all" : "" };
  return searchUrlFromFilters(next);
}

export function SearchFilterChips({ filters }: { filters: SearchFilters }) {
  const chips: { label: string; href: string }[] = [];

  if (filters.q?.trim()) {
    chips.push({ label: `"${filters.q.trim()}"`, href: withoutFilter(filters, "q") });
  }
  if (filters.crimeCategory) {
    chips.push({
      label: CRIME_CATEGORY_LABELS[filters.crimeCategory],
      href: withoutFilter(filters, "crimeCategory"),
    });
  }
  if (filters.psychologicalFactor) {
    chips.push({
      label: FACTOR_LABELS[filters.psychologicalFactor],
      href: withoutFilter(filters, "psychologicalFactor"),
    });
  }
  if (filters.theoreticalFramework) {
    chips.push({
      label: FRAMEWORK_LABELS[filters.theoreticalFramework],
      href: withoutFilter(filters, "theoreticalFramework"),
    });
  }
  if (filters.diagnosis?.trim()) {
    chips.push({ label: filters.diagnosis.trim(), href: withoutFilter(filters, "diagnosis") });
  }
  if (filters.country) {
    chips.push({
      label: COUNTRY_LABELS[filters.country],
      href: withoutFilter(filters, "country"),
    });
  }
  if (filters.location?.trim()) {
    chips.push({ label: filters.location.trim(), href: withoutFilter(filters, "location") });
  }
  if (filters.period?.trim()) {
    chips.push({ label: filters.period.trim(), href: withoutFilter(filters, "period") });
  }
  if (filters.offenderSex) {
    chips.push({
      label: filters.offenderSex === "unknown" ? "Sex unknown" : filters.offenderSex,
      href: withoutFilter(filters, "offenderSex"),
    });
  }
  if (filters.documentType) {
    chips.push({
      label: DOCUMENT_TYPE_LABELS[filters.documentType],
      href: withoutFilter(filters, "documentType"),
    });
  }
  if (filters.status) {
    chips.push({
      label: filters.status.charAt(0).toUpperCase() + filters.status.slice(1),
      href: withoutFilter(filters, "status"),
    });
  }
  if (filters.catalogTier && filters.catalogTier !== "all") {
    const catalogLabels: Record<string, string> = {
      curated: "Curated only",
      composite: "Composite archive",
    };
    chips.push({
      label: catalogLabels[filters.catalogTier] ?? filters.catalogTier,
      href: withoutFilter(filters, "catalogTier"),
    });
  }

  if (!chips.length) return null;

  return (
    <div className="active-filters mt-4 flex flex-wrap items-center gap-2" aria-label="Active filters">
      <span className="text-xs font-medium uppercase tracking-wide text-[var(--muted)]">Filters</span>
      {chips.map((chip) => (
        <Link key={chip.label} href={chip.href} className="filter-chip filter-chip-removable">
          {chip.label}
          <span aria-hidden> ×</span>
        </Link>
      ))}
      <Link href="/search" className="filter-chip filter-chip-clear">
        Clear all
      </Link>
    </div>
  );
}
