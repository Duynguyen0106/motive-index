...
async function main() {
  ...
  const {
    ...
    shouldIndexCase,
  } = await import(pathToFileURL(path.join(root, "src/lib/casePublishState.ts")).href);
  const { validateProvenance } = await import(
    pathToFileURL(path.join(root, "src/lib/validation/caseProvenance.ts")).href
  );
